...
%    The Zilog Z80 is an 8-bit microprocessor that used to be ubiquitious in personal computing and arcade gaming in the 1970-80's. We introduce ZC, a C compiler for the eZ80 variant of the Z, is still used to this day in Texas Instrument's TI-83/84+ graphing calculators. More recently, the eZ80 processor, introduced in the early 2000's, features a minor, backwards-compatible update to the original Z80 ISA, and is used in the TI's higher-end calculators, e.g. the TI-84+ CE. TI calculator programmers (including me), 
    ...
    \innerblock{The Zilog Z80 \& eZ80}{
      ...
      \item Backwards-compatible eZ80 adds several new instructions to the Z80 ISA
      \end{itemize}
    }
  \end{minipage}
  \begin{minipage}{0.5\linewidth}
    \innerblock{Motivation}{
      ...
      \item Most calculator programming done in Z80 assembly
  ...
  \end{minipage}
  To fulfill this need, I wrote {\bf ZC}, a C compiler for the eZ80 architecture optimizing for code size.
  }
  ...
  \block{Compiler Anatomy}{
    ...
    \innerblock{Frontend}{
      ...
      ZC infers the types of expressions in the AST,
    ...
    \innerblock{Backend}{
      {\bf Code Generation:}
      For each function, ZC generates a control flow diagram of
  ...
  \block{Register Allocation} {
    ...
    \begin{minipage}{0.5\linewidth}
      \innerblock{Stack Machine}{
        A stack-machine compiler pushes intermediate values onto the stack and pops them
        off when they are ready to be used.
    ...
    \begin{minipage}{0.5\linewidth}
      \innerblock{Register Allocation}{
        A compiler with a register allocator stores intermediate values into registers
        rather than pushing them onto the stack.
        However, there are often more intermediate values
        than there are physical registers.
        For example, the (e)Z80 has {\bf 7 single-byte registers}
        but only {\bf 3 allocatable multi-byte registers}.
    ...
    \begin{minipage}{0.5\linewidth}
      \innerblock{Frame vs. Stack Spilling in ZC}{
        Most compilers use {\it frame spilling}, i.e. spill intermediate values to
        the activation record.
      However, accessing memory through the frame pointer register
      {\tt ix} is costly in (e)Z80;
      {\it stack spilling}, i.e. to push/pop intermediate values to/from the stack,
      is more efficient.
      Stack spilling is {\bf cheap but restrictive};
      frame spilling is {\bf costly but versatile}.
    }
    \end{minipage}
  ...
  \block{Performance}{
    ...
    though each compiler more compact code in a subset of programs.
