...
namespace zc::z80 {
   ...
   ByteRegister r_a("a"), r_b("b"), r_c("c"), r_d("d"), r_e("e"), r_f("f"), r_h("h"),
      r_l("l"), r_ixh("ixh"), r_ixl("ixl"), r_iyh("iyh"), r_iyl("iyl");

   MultibyteRegister r_af(MultibyteRegister::ByteRegs{&r_a, &r_f}, "af"),
   ...
   RegisterValue rv_a(&r_a), rv_b(&r_b), rv_c(&r_c), rv_d(&r_d), rv_e(&r_e), rv_f(&r_f),
      rv_h(&r_h), rv_l(&r_l), rv_ixh(&r_ixh), rv_ixl(&r_ixl), rv_iyh(&r_iyh), rv_iyl(&r_iyl);
   RegisterValue rv_af(&r_af), rv_bc(&r_bc), rv_de(&r_de), rv_hl(&r_hl), rv_ix(&r_ix),
   ...
   bool Register::Eq(const Register *other) const {
      return name() == other->name() && size() == other->size() && Eq_(other);
   }
